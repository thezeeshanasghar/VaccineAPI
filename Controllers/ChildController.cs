       ...
       public async Task<Response<IEnumerable<ScheduleDTO>>> GetChildSchedule(int id)
       {
        ...
        private Stream CreateSchedulePdf(int childId)
        {
            ...
            var document = new Document(PageSize.A4, 45, 45, 30, 30);
            {
                ...
                float[] widths = new float[] { 30f, 70f, 135f, 80f, 60f, 70f, 60f, 60f };
               ...
               Font GetStatusFont(string status)
                ...
                foreach (var group in groupedSchedules)
                {
                    ...
                    // Determine group-level status (use first schedule's status for the group)
                    var firstSchedule = groupList[0];
                    string groupStatus;
                    Font groupStatusFont;
                    if (firstSchedule.IsDone == true && firstSchedule.IsDisease != true && firstSchedule.Due2EPI != true)
                    { groupStatus = "Given"; groupStatusFont = boldfont1; }
                    else if (firstSchedule.IsDone == true && firstSchedule.IsDisease != true && firstSchedule.Due2EPI == true)
                    { groupStatus = "By EPI"; groupStatusFont = font; }
                    else if (firstSchedule.IsDone == false && firstSchedule.IsDisease != true && !checkForMissed(firstSchedule.Date))
                    { groupStatus = "Due"; groupStatusFont = font; }
                    else if (firstSchedule.IsDone == false && firstSchedule.IsDisease != true && checkForMissed(firstSchedule.Date))
                    { groupStatus = "Missed"; groupStatusFont = italicfont1; }
                    ...
                    // Group date label
                    string groupDateLabel;
                    if (firstSchedule.IsDisease == true)
                        groupDateLabel = firstSchedule.Date.Date.ToString("yyyy") + " Y";
                    else if (firstSchedule.IsDone == true && firstSchedule.IsDisease != true && firstSchedule.Due2EPI != true)
                        groupDateLabel = firstSchedule.GivenDate?.ToString("dd/MM/yyyy") ?? firstSchedule.Date.Date.ToString("dd/MM/yyyy");
                    else
                        groupDateLabel = firstSchedule.Date.Date.ToString("dd/MM/yyyy");

                    if (!string.IsNullOrEmpty(groupDateLabel) && firstSchedule.IsDone == true)
                        upperTableDates.Add(groupDateLabel);

                    // Build merged Weight/Height/OFC cells using first done schedule in group
                    var doneSchedule = groupList.FirstOrDefault(s => s.IsDone == true && s.IsDisease != true && s.Due2EPI != true);
                    Paragraph pw = new Paragraph("", rangevaluefont);
                    Paragraph ph = new Paragraph("", rangevaluefont);
                    Paragraph pc = new Paragraph("", ofcValueFont);
                    if (doneSchedule != null)
                    ...
                    foreach (var dbSchedule in groupList)
                    {
                        count++;

                        // Sr# — merged, first row only
                        if (isFirstRow)
                        {
                            PdfPCell srCell = new PdfPCell(new Phrase(srNo.ToString(), font));
                            srCell.Rowspan = rowSpanCount;
        ...
        public Response<ChildDTO> Post(ChildDTO childDTO)
        {
            ...
            childDTO.FatherName = textInfo.ToTitleCase(childDTO.FatherName);
            {
                ...
                if (childDTO.Type == "regular")
                {
                    ...
                    {
                        // ── EPI Plus registration — bracket-aware schedule creation ──────────
                        var epiDob = childDTO.DOB.Date;
                        var epiToday = DateTime.UtcNow.AddHours(5).Date;
                        var epiAgeInDaysAtReg = (epiToday - epiDob).Days;

                        var epiD2000 = new DateTime(2000, 1, 1);
                        var epiD2002 = new DateTime(2002, 1, 1);
                        var epiD2009 = new DateTime(2009, 1, 1);
                        var epiD2014 = new DateTime(2014, 1, 1);
                        var epiD2021 = new DateTime(2021, 1, 1);
                        ...
                        if (epiDob >= epiD2000 && epiDob < epiD2002)
                        {
                            EpiAdd(148, 42); EpiAdd(149, 70); EpiAdd(150, 98); // OPV+DPT+HBV (no Hib)
                        }
                        else if (epiDob >= epiD2002 && epiDob < epiD2009)
                        {
                            EpiAdd(34, 42); EpiAdd(35, 70); EpiAdd(36, 98);
                        }
                        else if (epiDob >= epiD2009 && epiDob < epiD2014)
                        {
                            EpiAdd(34, 42); EpiAdd(35, 70); EpiAdd(36, 98);
                            EpiAdd(41, 42); EpiAdd(42, 70); EpiAdd(43, 98); // PCV 1/2/3
                        }
                        else if (epiDob >= epiD2014)
                        {
                            EpiAdd(34, 42); EpiAdd(35, 70); EpiAdd(36, 98);
                            EpiAdd(41, 42); EpiAdd(42, 70); EpiAdd(43, 98); // PCV 1/2/3
                            if (epiAgeInDaysAtReg <= 111) { EpiAdd(64, 42); EpiAdd(65, 70); } // Rota 1/2
                        }

                        EpiAdd(113, 270); // MR 1 at 9 months
                        EpiAdd(114, 456); // MR 2 at 15 months
                        if (epiDob >= epiD2021) EpiAdd(30, 270); // TCV at 9 months

                        // Create EPI schedules (IsDone=true, Due2EPI=true)
                        foreach (var kvp in epiGapDays)
                        {
                            var scheduledDate = epiDob.AddDays(kvp.Value);
                        ...
                        if (!epiGavePCV)
                        {
                            if      (epiAgeInDaysAtReg < 182)  { clinicDoseIds.Add(41); clinicDoseIds.Add(42); clinicDoseIds.Add(43); clinicDoseIds.Add(66); }
                            else if (epiAgeInDaysAtReg < 365)  { clinicDoseIds.Add(41); clinicDoseIds.Add(42); clinicDoseIds.Add(66); }
                        ...
                        clinicDoseIds.Add(60);  // MenACWY 1
                        clinicDoseIds.Add(59);  // MenACWY 2
                        ...
                        // Infinite vaccines (Flu, Typhoid, Vitamin A)
                        foreach (var kvp in doctorDoses)
                        {
                            ...
                            if (isVitaminA && epiAgeInDaysAtReg > 1825) continue;
                            if (isTyphoid && epiDob >= epiD2021 && dbDose.DoseOrder == 1) continue;

                            _db.Schedules.Add(new Schedule
                            {
                                ChildId     = childDTO.Id,
                                DoseId      = doseId,
                                Date        = calculateDate(childDTO.DOB, ds.GapInDays),
                                IsDone      = false,
                                Due2EPI     = false,
                                DiseaseYear = ""
                            });
        ...
        public IActionResult DownloadInvoicePDFUpdated(int Id, DateTime ScheduleDate, DateTime InvoiceDate, int ConsultationFee)
        {
            ...
            // Save exact PDF bytes to disk so parent gets byte-for-byte identical copy
            var invoice = _db.Invoices
                .Where(i => i.ChildId == Id)
                .Join(_db.Schedules.Where(s => s.ChildId == Id && s.Date.Date == ScheduleDate.Date),
                      i => i.DoseId, s => (long)s.Dose.Id, (i, s) => i)
                .FirstOrDefault();
            if (invoice != null)
            {
                var invoiceDir = Path.Combine(Directory.GetCurrentDirectory(), "Resources", "Invoices");
                Directory.CreateDirectory(invoiceDir);
                System.IO.File.WriteAllBytes(Path.Combine(invoiceDir, $"{invoice.InvoiceId}.pdf"), output.ToArray());
            }
        ...
        // Parent download — serves the exact saved PDF bytes, no re-generation
        [HttpGet("{Id}/{ScheduleDate}/invoice-file")]
        public IActionResult GetInvoiceFile(int Id, DateTime ScheduleDate)
        {
            var invoice = _db.Invoices
                .Where(i => i.ChildId == Id)
                .Join(_db.Schedules.Where(s => s.ChildId == Id && s.Date.Date == ScheduleDate.Date),
            ...
            if (invoice == null)
                return NotFound(new { message = "Invoice not yet generated by clinic." });

            var pdfPath = Path.Combine(Directory.GetCurrentDirectory(), "Resources", "Invoices", $"{invoice.InvoiceId}.pdf");
            if (!System.IO.File.Exists(pdfPath))
                return NotFound(new { message = "Invoice file not found. Please ask the clinic to re-download the invoice." });

            var bytes = System.IO.File.ReadAllBytes(pdfPath);
            return File(bytes, "application/pdf", $"Invoice_{invoice.InvoiceId}.pdf");
        }
