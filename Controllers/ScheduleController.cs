    ...
    public class ScheduleController : ControllerBase
    {
        ...
        public IActionResult AddSchedule(long DoctorId, long ChildId)
        {
            try
            {
                ...
                // Group doses by vaccine, enforce MinGap within each vaccine's dose sequence
                var dosesByVaccine = dbDoses
                    .GroupBy(d => d.VaccineId)
                    .ToList();

                foreach (var vaccineGroup in dosesByVaccine)
                {
                    var orderedDoses = vaccineGroup.OrderBy(d => d.DoseOrder).ToList();
                    DateTime previousDate = DateTime.MinValue;

                    foreach (var dbDose in orderedDoses)
                    {
                        var minAgeDate = calculateDate(dbChild.DOB, dbDose.MinAge);

                        DateTime scheduledDate;
                        if (previousDate == DateTime.MinValue)
                        {
                            scheduledDate = minAgeDate;
                        }
